...
public class NauseaEnchant extends Enchant {

    @ConfigValue(path = "Enchant.Tipsy.chance.level 1")
    public double level1 = 0.1;
    @ConfigValue(path = "Enchant.Tipsy.chance.level 2")
    public double level2 = 1;
    @ConfigValue(path = "Enchant.Tipsy.chance.level 3")
        ...
        if (e.getEntity() instanceof Player && e.getDamager() instanceof Player) {
            ...
            if (leggings != null && core.enchantManager.getEnchants(leggings).contains(this)) {
                ...
                if (Math.random() * 100 < getChance(level))
                    ((Player) e.getDamager()).addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION, level * 60, level - 1));
