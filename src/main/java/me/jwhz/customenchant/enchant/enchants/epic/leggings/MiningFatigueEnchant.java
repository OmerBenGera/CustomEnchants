...
public class MiningFatigueEnchant extends Enchant {

    @ConfigValue(path = "Enchant.Weiry.chance.level 1")
    public double level1 = 0.1;
    @ConfigValue(path = "Enchant.Weiry.chance.level 2")
    public double level2 = 1;
    @ConfigValue(path = "Enchant.Weiry.chance.level 3")
        ...
        if (e.getEntity() instanceof Player && e.getDamager() instanceof Player) {
            ...
            if (leggings != null && core.enchantManager.getEnchants(leggings).contains(this)) {
                ...
                if (Math.random() * 100 < getChance(level))
                    ((Player) e.getDamager()).addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, level * 60, 0));
