...
public class WeaknessEnchant extends Enchant {

    @ConfigValue(path = "Enchant.Fortify.chance.level 1")
    public double level1 = 0.1;
    @ConfigValue(path = "Enchant.Fortify.chance.level 2")
    public double level2 = 1;
    @ConfigValue(path = "Enchant.Fortify.chance.level 3")
    public double level3 = 10;
    @ConfigValue(path = "Enchant.Fortify.duration in ticks")
        ...
        if (e.getEntity() instanceof Player && e.getDamager() instanceof Player) {
            ...
            if (chestplate != null && core.enchantManager.getEnchants(chestplate).contains(this)) {
                ...
                if (Math.random() * 100 < getChance(level))
                    ((Player) e.getDamager()).addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, duration, 0));
